...
s.connect((Client.TCP_IP, Client.TCP_PORT))

infoAgent = Client.pokerGames()

while True:

    try:
        # Get data
        data = s.recv(Client.BUFFER_SIZE)
        ...
        if RequestType == 'Name?': # if Server request for name
            s.send('Name ' + infoAgent.queryPlayerName(Client.POKER_CLIENT_NAME) + "\n")
        ...
        # * Append space, the players name, space and the amount of chips after this string. Separate the words by space. */
            ...
            if MsgFractions[1] == Client.POKER_CLIENT_NAME:
                infoAgent.Chips = int(MsgFractions[2])
            else:
                infoAgent.infoPlayerChips(MsgFractions[1], MsgFractions[2])
        ...
        # * Append space and the value of the ante after this string. */
            ...
            infoAgent.infoAnteChanged(MsgFractions[1])
        ...
        # * Append the players name and the bet value after this string. Separate the words by space. */
            ...
            if MsgFractions[1] == Client.POKER_CLIENT_NAME:
                ...
                infoAgent.infoForcedBet(MsgFractions[1], MsgFractions[2])
        ...
        # * Separate the words by space. */
            ...
            tmp = infoAgent.queryOpenAction(minimumPotAfterOpen, playersCurrentBet, playerRemainingChips)
            if isinstance( tmp, str ): # For check and All-in
                s.send(tmp + "\n")
            elif len(tmp) == 2: # For open
                s.send(tmp[0] + ' ' + str(tmp[1]) + " \n")
            print(SIGNAL_ALIVE)
            print(Client.POKER_CLIENT_NAME + 'Action>', tmp)
            ...
            tmp = infoAgent.queryCallRaiseAction(maximumBet, minimumAmountToRaiseTo, playersCurrentBet, playersRemainingChips)
            ...
            infoAgent.infoPlayerHand(Client.POKER_CLIENT_NAME, infoAgent.CurrentHand)
            #print('CurrentHand>', infoAgent.CurrentHand)
        elif RequestType == 'Draw?':
            discardCards = infoAgent.queryCardsToThrow(infoAgent.CurrentHand)
            s.send('Throws ' + discardCards + "\n")
            print(Client.POKER_CLIENT_NAME + ' Action>' + 'Throws ' + discardCards)
        ...
        # * Separate the words by space. */
            ...
            infoAgent.infoPlayerOpen(MsgFractions[1], MsgFractions[2])
        ...
        # * Append the players name after this string. Separate the words by space. */
            ...
            infoAgent.infoPlayerCheck(MsgFractions[1])
        ...
        # * Append the players name and the number of cards exchanged after this string. Separate the words by space. */
            ...
            infoAgent.infoPlayerDraw(MsgFractions[1], MsgFractions[2])
        ...
        # * Append the players name and the amount of chips the player won after the string. Separate the words by space. */
            ...
            infoAgent.infoRoundUndisputedWin(MsgFractions[1], MsgFractions[2])
        ...
        # * Append the players name and the amount of chips the player won after the string. Separate the words by space. */
            ...
            infoAgent.infoRoundResult(MsgFractions[1], MsgFractions[2])
        ...
        # * Append the players name and the cards of the players hand after this string. Separate the words by space.*/
            ...
            infoAgent.infoPlayerHand(MsgFractions[1], MsgFractions[2:])
